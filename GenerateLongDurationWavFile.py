...
class Gen():
    ...
    def Generate(self):
        ...
        self.save_wav("outputLongDur.wav")
    ...
    def Set_led(self,SampleLeds): 
        ...
        for root, _dirs, files in os.walk('.'):
            for name in files:
                if fnmatch.fnmatch(name, "outputLongDur.wav"):
                    paths.append(os.path.join(root, name))
        
        print paths
        
        if len(paths)>0:
            SampleLeds.Gen_longdurationquality_audio.led.canvas.itemconfig(SampleLeds.Gen_longdurationquality_audio.led.circle, fill="green") 
